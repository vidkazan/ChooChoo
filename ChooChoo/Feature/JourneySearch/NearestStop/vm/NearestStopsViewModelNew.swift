...
class NearestStopsViewModelNew : ChewViewModelProtocol {
...
//    func sendCode() async {
//        await state.setState(newState: .inProgress)
//        if isPhoneNumberValid() {
//            let clearNumber = trimmingWrongCharacters(phone: phoneNumber)
//            let result = await userRepository.sendCode(phone: clearNumber)
//            switch result {
//            case .success:
//                router.push(.sms)
//                await state.setState(newState: .success)
//            case .failure(let failure):
//                store.dispatch(ErrorBannerAction.show(failure))
//                await state.setState(newState: .idle)
//            }
//        }
//    }
}

extension NearestStopsViewModelNew {
    struct State {
		let data : StateData
		let status : Status
	}
...
extension NearestStopsViewModelNew {
    ...
    func whenLoadingNearbyStops() -> Feedback<State, Event> {
    ...
    func whenLoadingStopDetails() -> Feedback<State, Event> {
        Feedback { (state: State) -> AnyPublisher<Event, Never> in
    ...
    func fetchLocatonsNearby(coords : CLLocation) -> AnyPublisher<[StopDTO],ApiError> {
    ...
    func fetchStopDepartures(stop : Stop) -> AnyPublisher<StopTripsDTO,ApiError> {
